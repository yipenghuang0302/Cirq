...
def make_order_finding_circuit(x: int, n: int) -> cirq.Circuit:
    ...
    return cirq.Circuit(
        ...
        cirq.decompose(ModularExp(target, exponent, x, n)),
        cirq.decompose(cirq.QFT(*exponent, inverse=True)),
...
def quantum_order_finder(x: int, n: int) -> Optional[int]:
    ...
    print(circuit)
    result = cirq.KnowledgeCompilationSimulator(circuit).run(circuit)
