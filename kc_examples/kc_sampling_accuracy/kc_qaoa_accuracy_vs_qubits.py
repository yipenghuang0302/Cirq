...
def main(
    ...
    for n in range (4,32,2):
        ...
        kc_chisqs[n]=[]
        dm_chisqs[n]=[]
        kc_power_divergences[n]=[]
        dm_power_divergences[n]=[]
        ks_2samps[n]=[]

        for print_n in range(4,n,2):
            # for kc_chisq,dm_chisq,kc_power_divergence,dm_power_divergence,ks_2samp in zip(kc_chisqs[print_n],dm_chisqs[print_n],kc_power_divergences[print_n],dm_power_divergences[print_n],ks_2samps[print_n]):
            for kc_power_divergence,dm_power_divergence,ks_2samp in zip(kc_power_divergences[print_n],dm_power_divergences[print_n],ks_2samps[print_n]):
                print(
                    "print_n="+str(print_n)+
        ...
        for _ in range(4):
            ...
            def f(x):
                ...
                # print("kc_sim_result.final_state_vector")
                # print(kc_sim_result.final_state_vector)
                # print("dm_sim_result.final_state_vector")
                # print(dm_sim_result.final_state_vector)
                # np.testing.assert_almost_equal(
                #     kc_sim_result.final_state_vector,
                #     dm_sim_result.final_state_vector,
                #     decimal=4
                # )
                ...
                # kc_chisqs[n].append(kc_chisq)
                # dm_chisqs[n].append(dm_chisq)
                # kc_power_divergences[n].append(kc_power_divergence)
                # dm_power_divergences[n].append(dm_power_divergence)
                kc_power_divergences[n].append(kc_power_divergence / 2 / repetitions)
                dm_power_divergences[n].append(dm_power_divergence / 2 / repetitions)
                ks_2samps[n].append(ks_2samp)
