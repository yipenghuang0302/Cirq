...
def _estimate_pauli_traces_general(qubits: List[cirq.Qid],
    ...
    dense_simulator = cirq.KnowledgeCompilationSimulator(circuit)
...
def direct_fidelity_estimation(circuit: cirq.Circuit, qubits: List[cirq.Qid],
    ...
    Args:
        ...
        samples_per_term: if set to 0, we use the 'sampler' parameter above as
            a noise (must be of type cirq.KnowledgeCompilationSimulator) and
    ...
    if samples_per_term == 0:
        ...
        if not isinstance(sampler, cirq.KnowledgeCompilationSimulator):
            raise TypeError('sampler is not a cirq.KnowledgeCompilationSimulator '
                            'but samples_per_term is zero.')
        noisy_simulator = cast(cirq.KnowledgeCompilationSimulator, sampler)
